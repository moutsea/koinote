...
- 会员可使用 AI 优化像代码审查一样查看标题与正文 Diff，逐条应用/忽略或一次处理全部；标题显示 0–100 吸引力评分，低于 60 时给出 2–3 个可替换标题。审阅提交后在后台运行，完成后通过站内通知返回结果
...
开始优化后，标题与开头、正文分段、结构排版会作为独立子任务在后台有限并发执行，正文较长时会继续按
Markdown 块拆分，但同时最多调用 3 路模型。进度和已完成的部分建议会持久化，因此切换页面或刷新后仍可查看；
某一路返回无效结果时只重试该子任务，不会重新生成整篇审阅。全部完成前，部分建议保持只读，避免应用一半后
让剩余结果基于过期正文。为控制超长文的成本，结构排版轨最多携带 32 KiB 实际块文本和 400 个块元数据，
按开头、结尾及正文均匀采样评估；正文内容优化仍按分段覆盖全文。
