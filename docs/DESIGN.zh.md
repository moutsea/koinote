...
AES-GCM 加密。发布时仍复用同一套微信 HTML 构建器，但不在正文重复标题；后端对正文图片去重后，以 2 个 worker 下载、校验和压缩，再以最多 4 个 worker 并发调用 `media/uploadimg`，避免多图串行上传耗尽源站响应时间。上传结果按原始位置写回，任何一次失败都会取消其余上传并保留原始错误；全部成功后才上传永久封面素材并调用 `draft/add`。日志只记录图片数量、阶段耗时和成功状态，不记录图片地址或令牌。封面设置支持默认封面、正文图片和 AI 生成三种来源；默认封面由客户端用 Koinote Logo 与文章标题生成，正文图片由服务端重新读取、校验并按选择的比例居中裁切，AI 封面不是同步前置条件。远程图片下载
