...
读写令牌还暴露 `sync_document_to_feishu`。它要求账号已在「设置 → 飞书」完成 OAuth 绑定，复用网页端的
飞书账号锁、限速器、凭证刷新、图片上传和关联表，按 Koinote 标题与正文创建或更新飞书文档。同步是单向的，
不读取或合并飞书侧修改；飞书绑定仍只能通过网页或桌面端完成，MCP 不处理 OAuth 回调。
...
基础格式在客户端完成；微信公众号草稿和知乎发布会把经过用户确认的富文本交给后端调用平台 API：
...
网页端和桌面端共用一条直接草稿链路：每个用户最多绑定 5 个公众号，可设置一个默认账号，并在同步草稿时选择目标账号。旧单账号接口仍映射到默认账号，兼容已发布客户端。后端用 `stable_token`
验证每个 AppID / AppSecret，access token 按账号隔离缓存；AppSecret 使用独立的 `WECHAT_CREDENTIAL_ENCRYPTION_KEY` 做
AES-GCM 加密。发布时仍复用同一套微信 HTML 构建器，但不在正文重复标题；后端对正文图片去重后，以 2 个 worker 下载、校验和压缩，再以最多 2 个 worker 并发调用 `media/uploadimg`，单次上传限制为 90 秒，以适应中转带宽。上传结果按原始位置写回，任何一次失败都会取消其余上传并保留原始错误；全部成功后才上传永久封面素材并调用 `draft/add`。日志只记录图片数量、阶段耗时和成功状态，不记录图片地址或令牌。封面设置支持默认封面、正文图片和 AI 生成三种来源；默认封面由客户端用 Koinote Logo 与文章标题生成，正文图片由服务端重新读取、校验并按选择的比例居中裁切，AI 封面不是同步前置条件。远程图片下载
复用私网/回环地址阻断、DNS 解析校验、禁止重定向和响应体上限，避免把草稿接口变成 SSRF 原语。
草稿失败时会尽力删除刚上传的孤立封面素材。

正文图片上传结果保存在 `wechat_image_upload_cache`（迁移 `0053`），按用户所属公众号、AppID 和压缩后图片的 SHA-256 隔离；同批不同地址但内容相同的图片也只上传一次。每次仍重新下载、校验和压缩原图，因此地址不变但内容变化时会重新上传，封面裁切和现有画质不变。缓存有效期为 30 天，发生上传后清理过期记录并只保留该账号最近 2,000 条；解绑账号或注销用户时级联删除。缓存读写最多等待 2 秒，失败会退回正常上传，不阻断草稿。每张图成功后即保存结果，后续图片失败时仍可在重试中复用已完成的图片。缓存命中数、需要上传的数量与字节数写入日志，图片地址和令牌不进入日志。缓存主要减少重复同步和失败重试的上传耗时，首次同步仍需完整传图；永久封面素材继续单独上传和回收。

微信公众号草稿的长请求由 Worker 与后端协商 `application/x-koinote-wechat-draft` 响应格式：后端在图片处理期间每 10 秒发送一条心跳，最后发送包含真实 HTTP 状态码和 JSON 正文的结果；Worker 限制响应读取量为 64 KiB，并还原普通 JSON 响应，因此已发布客户端无需升级。缺失最终结果或流中断按失败处理，不会将心跳当成创建成功。整个草稿处理限制为 10 分钟，客户端断线继续沿请求上下文取消上传，失败按原逻辑释放 credits。微信 CONNECT 中转按连续两分钟无读写活动关闭连接，不能以连接建立时间截断仍在传输的请求。
...
| **不**剥 `background`（实测确认） | 保留整篇底色的主题可以原样存活      |

后两条是靠用户反复粘贴测出来的，不是查文档得来的 —— 微信没有公开的过滤规则清单，
只能一条条试。记在这里是因为它们决定了下面两处实现。

主题的 `body` 规则通常会作为最外层 `<section>` 的样式导出，因此主题背景默认会保留。
`verge`、`stripe`、`ft`、`linear`、`notion` 以及四套新主题 `paper`、`signal`、
`notes`、`pulse` 通过 `exportBodyBackground: false` 关闭整篇背景，避免在公众号白色
阅读区铺出一整块色块；标题、引用、代码块等局部元素的背景仍按主题规则保留。
其中 `linear` 原本使用深色正文文字，导出时额外通过 `exportBodyColor` 换成适合白底的
颜色，避免去掉深色背景后对比度不足。
...
所以这里的主题都不依赖伪元素。唯一保留的后代选择器是 `pre code`（代码块内的
`code` 与行内 `code` 视觉不同）。
