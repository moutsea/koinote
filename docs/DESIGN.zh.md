...
AES-GCM 加密。发布时仍复用同一套微信 HTML 构建器，但不在正文重复标题；后端把正文图片逐张
下载、校验并通过 `media/uploadimg` 转存，再上传永久封面素材并调用 `draft/add`。封面设置支持默认封面、正文图片和 AI 生成三种来源；默认封面由客户端用 Koinote Logo 与文章标题生成，正文图片由服务端重新读取、校验并按选择的比例居中裁切，AI 封面不是同步前置条件。远程图片下载
