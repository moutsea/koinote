...
type EmailPurpose = "registration" | "password_reset";
...
> = {
  ...
  password_reset: {
    en: {
      subject: "Reset your Koinote password",
      heading: "Reset your password",
      body: "Use the code below to choose a new password for your Koinote account.",
      expires: "This code expires in 10 minutes.",
      ignore:
        "If you did not request this, you can ignore this email. Your password has not changed.",
    },
    zh: {
      subject: "重置你的 Koinote 密码",
      heading: "重置密码",
      body: "请使用以下验证码为你的 Koinote 账号设置新密码。",
      expires: "验证码将在 10 分钟后失效。",
      ignore: "如果不是你本人操作，请忽略此邮件。你的密码尚未发生变化。",
    },
    fr: {
      subject: "Réinitialisez votre mot de passe Koinote",
      heading: "Réinitialisez votre mot de passe",
      body: "Utilisez le code ci-dessous pour choisir un nouveau mot de passe Koinote.",
      expires: "Ce code expire dans 10 minutes.",
      ignore:
        "Si vous n’êtes pas à l’origine de cette demande, ignorez cet e-mail. Votre mot de passe n’a pas changé.",
    },
    ja: {
      subject: "Koinote パスワードの再設定",
      heading: "パスワードを再設定",
      body: "以下のコードを使用して Koinote の新しいパスワードを設定してください。",
      expires: "このコードは 10 分後に期限切れになります。",
      ignore:
        "心当たりがない場合は、このメールを無視してください。パスワードは変更されていません。",
    },
  },
};
...
function emailContent(code: string, locale: Locale, purpose: EmailPurpose) {
  const message = copy[purpose][locale];
  return {
...
export async function handleVerificationEmail(
  ...
  const purpose: EmailPurpose =
    body.purpose === "password_reset" ? "password_reset" : "registration";
  if (
    !/^\S+@\S+\.\S+$/.test(email) ||
    email.length > 255 ||
    !/^\d{6}$/.test(code)
  ...
  const content = emailContent(code, normalizeLocale(body.locale), purpose);
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      await env.EMAIL.send({
        ...
        from: {
          email: fromAddress,
          name: env.EMAIL_FROM_NAME?.trim() || "Koinote",
        },
